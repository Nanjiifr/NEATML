...
type t = {
  ...
  mutable bias : float array;
      (* depth - keeping as float array for simplicity or convert to Tensor? 
...
let create input_depth input_height input_width kernel_size output_depth
    ...
    let limit =
      sqrt
        ...
        /. float
             ...
             + (output_depth * kernel_size * kernel_size)))
    in
  ...
  in
  let kernels =
    Array.init output_depth (fun _ ->
        Array.init input_depth (fun _ -> init_k ()))
  in
  let bias = Array.make output_depth 0.0 in
  let grad_weights =
    Array.init output_depth (fun _ ->
        Array.init input_depth (fun _ ->
    ...
    for b = 0 to batch_size - 1 do
      ...
      for o = 0 to layer.output_depth - 1 do
        ...
        for i = 0 to layer.input_depth - 1 do
          let map_i_data =
            Array.make_matrix layer.input_height layer.input_width 0.0
          in
          ...
          for r = 0 to layer.input_height - 1 do
            for c = 0 to layer.input_width - 1 do
              map_i_data.(r).(c) <-
                input_row.(start_idx + (r * layer.input_width) + c)
            done
          done;
          let map_i =
            if !use_gpu then to_gpu (CPU map_i_data) else CPU map_i_data
          in
        ...
        for r = 0 to out_h - 1 do
          for c = 0 to out_w - 1 do
            outputs.(b).((o * out_h * out_w) + (r * out_w) + c) <-
              final_map.(r).(c) +. b_val
          done
        done
      done
...
let backward layer upstream_grad =
  ...
  let out_w = layer.input_width - layer.kernel_size + 1 in
  let d_inputs =
    Array.make_matrix batch_size
      (layer.input_depth * layer.input_height * layer.input_width)
      0.0
  in
  ...
  for b = 0 to batch_size - 1 do
    for o = 0 to layer.output_depth - 1 do
      ...
      let dz_map =
        if !use_gpu then to_gpu (CPU dz_map_data) else CPU dz_map_data
      in
      ...
      layer.grad_bias.(o) <-
        layer.grad_bias.(o) +. (!sum_bias /. float batch_size);
      ...
      for i = 0 to layer.input_depth - 1 do
        ...
        for r = 0 to layer.input_height - 1 do
          for c = 0 to layer.input_width - 1 do
            in_map_data.(r).(c) <-
              inputs.(b).(in_start + (r * layer.input_width) + c)
          done
        done;
        let in_map =
          if !use_gpu then to_gpu (CPU in_map_data) else CPU in_map_data
        in

        let dw = conv "valid" in_map dz_map in
        let scaled_dw = scalar (1. /. float batch_size) dw in
        layer.grad_weights.(o).(i) <-
          add_matrices layer.grad_weights.(o).(i) scaled_dw;

        let k = layer.kernels.(o).(i) in
        let k_cpu = to_cpu k in
        let k_flipped_data =
          Array.make_matrix layer.kernel_size layer.kernel_size 0.0
        in
        for r = 0 to layer.kernel_size - 1 do
          for c = 0 to layer.kernel_size - 1 do
            k_flipped_data.(r).(c) <-
              k_cpu.(layer.kernel_size - 1 - r).(layer.kernel_size - 1 - c)
          done
        done;
        let k_flipped =
          if !use_gpu then to_gpu (CPU k_flipped_data) else CPU k_flipped_data
        in
  ...
  {
    Gradients.d_input = CPU d_inputs;
    d_weights = Gradients.Conv layer.grad_weights;
    d_bias = CPU [| layer.grad_bias |];
  }
