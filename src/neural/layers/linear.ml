...
let create in_dim out_dim _batch activation =
  ...
  let w_data =
    Array.init out_dim (fun _ ->
        Array.init in_dim (fun _ -> Random.float (2.0 *. limit) -. limit))
  in
  ...
  let gw =
    if !use_gpu then GPU (Gpu.of_cpu (Array.make_matrix out_dim in_dim 0.0))
    else CPU (Array.make_matrix out_dim in_dim 0.0)
  in
  let gb =
    if !use_gpu then GPU (Gpu.of_cpu [| Array.make out_dim 0.0 |])
    else CPU [| Array.make out_dim 0.0 |]
  in
  {
    weights;
    bias;
...
let forward layer inputs =
  ...
  | GPU in_g, GPU w_g, GPU b_g when !use_gpu ->
      let out_g =
        Gpu.linear_fwd in_g w_g b_g (rows inputs) (rows layer.weights)
          (cols layer.weights)
          (act_to_int layer.activation)
      in
  ...
  | _ ->
      ...
      let b_cpu = to_cpu layer.bias in
      let z =
        Array.init (Array.length inputs_cpu) (fun b ->
            let row = Array.make (Array.length w_cpu) 0.0 in
            for i = 0 to Array.length w_cpu - 1 do
              let sum = ref b_cpu.(0).(i) in
              for k = 0 to Array.length inputs_cpu.(0) - 1 do
                sum := !sum +. (inputs_cpu.(b).(k) *. w_cpu.(i).(k))
              done;
              row.(i) <- !sum
            done;
            row)
...
let backward layer upstream =
  ...
  | ( true,
      ...
      let d_in_g =
        Gpu.linear_bwd up_g in_g w_g out_g gw_g gb_g batch out_dim in_dim act
      in
      {
        Gradients.d_input = GPU d_in_g;
        d_weights = Gradients.Dense layer.grad_weights;
        d_bias = layer.grad_bias;
      }
  | _ ->
      failwith "Linear.backward: CPU fallback not implemented in full GPU mode"
...
let zero_grad layer =
  ...
  | _ ->
      layer.grad_weights <-
        zeros (rows layer.grad_weights) (cols layer.grad_weights);
