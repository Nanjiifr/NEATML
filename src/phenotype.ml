...
let create_phenotype g =
  ...
  let inputs_list = List.filter (fun (n : node_gene) -> n.kind = Sensor) g.nodes |> List.map (fun (n : node_gene) -> n.id) |> List.sort compare in
  let outputs_list = List.filter (fun (n : node_gene) -> n.kind = Output) g.nodes |> List.map (fun (n : node_gene) -> n.id) |> List.sort compare in
  ...
  let neurons = Array.make num_neurons { value = 0.; kind = Hidden; incoming = [||] } in
  
  (* Build fast neurons *)
  (* First, build a map of incoming connections per node ID *)
  let incoming_map = Hashtbl.create num_neurons in
  List.iter (fun (c : connection_gene) ->
    if c.enabled then
      let current = try Hashtbl.find incoming_map c.out_node with Not_found -> [] in
      Hashtbl.replace incoming_map c.out_node (c :: current)
  ) g.connections;

  List.iter (fun (n : node_gene) ->
    let idx = Hashtbl.find id_to_idx n.id in
    let incoming_genes = try Hashtbl.find incoming_map n.id with Not_found -> [] in
    let incoming_arr = 
      incoming_genes 
      |> List.map (fun (c : connection_gene) -> 
          ...
          with Not_found -> 
            ...
            { src_idx = 0; weight = 0.0 } (* Dummy, potentially dangerous if 0 is used *)
      )
  ...
  let input_indices = List.map (Hashtbl.find id_to_idx) inputs_list |> Array.of_list in
  let output_indices = List.map (Hashtbl.find id_to_idx) outputs_list |> Array.of_list in
  
  (* Topo order for update: All non-sensors *)
  let update_indices_list = ref [] in
  for i = 0 to num_neurons - 1 do
    if neurons.(i).kind <> Sensor then update_indices_list := i :: !update_indices_list
  done;
...
let predict nn inputs_list =
  ...
  *)
  ...
  (* Gather outputs *)
  Array.map (fun idx -> nn.neurons.(idx).value) nn.output_indices |> Array.to_list
