...
let () =
  ...
  let x_train = Array.map (fun row -> Array.sub row 0 2) dataset in
  let y_train = Array.map (fun row -> Array.sub row 2 1) dataset in
  ...
  Optimizer.fit model x_train y_train x_train (* Test on same data for XOR *)
    y_train 4 (* Batch size *)
    1000 (* Epochs *)
    optimizers Errors.MSE;
